...
2. Проверьте содержимое ответа сервера. Вернул ли сервер содержимое файла явно? Как вы
   это можете увидеть?
   - Да
   - В ответе указано поле контента и сам контент
...
1. Сколько HTTP GET запросов было отправлено вашим браузером? На какие интернет-адреса были отправлены эти GET-запросы?
   - 3 запроса (не считая favicon.ico)
   - http://gaia.cs.umass.edu (128.119.245.12), 2.56.99.24
2. Можете ли вы сказать, загрузил ли ваш браузер два изображения последовательно или
   ...
   - Последовательно, так как сначала пришёл возврат с pearson.png, и только затем отправили запрос на 8E_cover_small.jpg (Но возможно всё-таки параллельно, но с большой задержкой отправки) 
