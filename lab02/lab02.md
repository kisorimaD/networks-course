...
![](images/my_put.png)
![](images/my_delete.png)
![](images/my_get_all_products.png)
...
![](images/my_post_icon.png)
![](images/my_get_icon.png)
...
Давайте следить за последним пакетом, именно когда он дойдет до приемника передачу можно считать оконченной.

Он проходит $N \dfrac{L}{R}$, когда начинает отправку, но до этого ещё ждет $(P - 1) \cdot \dfrac{L}{R}$ пока отправятся предыдущие пакеты

Значит итоговое время - $(N + P - 1) \cdot \dfrac{L}{R}$
...
Снова будем смотреть за последним пакетом. 

$A = 40 \hspace{0.5cm} R_1 = 0.2 \hspace{0.5cm} R_2 = 3 \hspace{0.5cm} R = 2 \hspace{0.5cm} L = \text{длина пакета в мегабитах}$ 

(приблизительно, так как отношения между килобайтом и мегабайтом не совсем 1000)

Он пройдет за время $\dfrac{L}{R_1} + \dfrac{L}{R_2} + \dfrac{L}{R_3}$ плюс время на ожидание $ \dfrac{(\frac{A}{L} - 1) \cdot L}{R_1} = \dfrac{A - L}{R_1}$

То есть

$\dfrac{5L}{6} + 200$ секунд

Передача займет примерно 200 секунд, при этом чем меньше размер пакета, тем быстрее передача (без учета заголовков)  
...
$$\mathbb{P}(X \geq 12) = \sum_{k=12}^{60} \binom{60}{k} 0.2^k 0.8^{60-k}$$

Примерное численное значение - 0.55138
...
Смотрим на последний пакет, его время:

$$3 \cdot \dfrac{S + 80}{R} + \dfrac{(\frac{X}{S} - 1)(80 + S)}{R}$$

То есть 

$$\dfrac{\frac{80X}{S} + 2S}{R} + \frac{X + 160}{R} $$

при заданном произведении сумма чисел минимальна при равенстве, то есть $80X = 2S^2$
...
Задержка передачи - это $\dfrac{L}{R}$

А полная задержка тогда

$$\dfrac{IL}{R(1-I)} + \dfrac{L}{R} = \dfrac{L}{R(1 - I)}$$

Отношение задержки к $\dfrac{L}{R(1 - I)}$ прямо пропорциональное, с коэффициентом $\dfrac{1}{1 - I}$